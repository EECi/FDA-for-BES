function [mu,gam_mu,psi,vec] = SqrtMeanMap(gam,muGates)
 
[n,T] = size(gam); % gam 24 x 505, transposed on entry to routine 
dT = 1/(T-1);
psi = zeros(n,T-1);
for i=1:n
    psi(i,:) = sqrt(diff(gam(i,:))/dT+eps); % psi values for new data
    %psi(i,:) = sqrt(diff(gam(i,:))+eps);
end

%Find direction
%mnpsi = mean(psi); % should be Gates mean
...
for iter = 1:maxiter
    for i=1:n
        ...
        dot1=trapz(linspace(0,1,T-1),mu.*psi(i,:));
        if dot1 > 1
            dot_limited = 1;
        elseif dot1 < -1
            dot_limited = -1;
